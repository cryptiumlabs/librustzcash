...
) -> edwards::Point<E, PrimeOrder>
{
    ...
    // The following loop exists because approximately half of field elements
    // exist as y coordinates on an elliptic curve, and therefore the group hash
    // may not exist for a given input tag. Additionally, there is an 
    // (extremely small) probability of hashing to a point in a
    // small order subgroup. Since the JubJub curve is the
    ...
    // loop runs approximately two times in expectation, to find a point at all. 
    // When it (rarely) hashes badly to a small order point , detect this by 
    // multiplying by the cofactor which gives the identity when the 
    // hash is in the small order subgroup. Similarly, the assertion is unlikely
    // to trigger
    loop {
        let gh = group_hash(
            &tag,
